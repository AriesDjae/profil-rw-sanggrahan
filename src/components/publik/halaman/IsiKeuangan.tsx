...
import { daftarRt, ringkasanLaporanPublik } from "@/lib/kueri";

import type { Lingkup } from "./lingkup";

/**
 * Laporan kas — dipakai `/keuangan` dan `/rw/[n]/keuangan`.
 *
 * Hanya laporan yang sudah lolos verifikasi Ketua RT dan persetujuan Ketua RW
 * yang tampil, sama seperti sebelumnya. Yang bertambah hanyalah batas RW:
 * laman RW 2 tidak pernah menampilkan kas RT milik RW 1 atau RW 3.
 */
export default async function IsiKeuangan({
  lingkup,
  rtId,
  tahunDiminta,
}: {
  ...
  const tahun = tahunDiminta ?? daftarTahun[0] ?? tahunSekarang;

  // RT dari RW lain diabaikan, bukan diteruskan ke kueri.
  const rtTerpilih = rtList.find((r) => r.id === rtId);
  const rtDipakai = rtTerpilih ? rtId : undefined;

  const ringkas = await ringkasanLaporanPublik({ rtId: rtDipakai, tahun, rwId });

  const totalPemasukan = ringkas.reduce((a, r) => a + r.pemasukan, 0);
  const totalPengeluaran = ringkas.reduce((a, r) => a + r.pengeluaran, 0);

  // Saldo terakhir tiap RT (laporan terurut terbaru dulu). Pada laman kampung
  // kuncinya harus menyebut RW juga: RT 01 ada di ketiga RW.
  ...
  const dasar = `${lingkup.basis}/keuangan`;
  const tautan = (opsi: { rt?: number | null; tahun?: number }) => {
    const q = new URLSearchParams();
    const rtBaru = opsi.rt === undefined ? rtDipakai : opsi.rt;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <JudulBagian
        ...
        judul={`Laporan Kas RT ${lingkup.nama}`}
        tingkat="h1"
        ...
        <ol className="mt-4 grid gap-4 sm:grid-cols-3">
          {[
            ...
            },
            {
              ...
              isi: "Persetujuan akhir diberikan Ketua RW yang menaungi RT tersebut, dan laporan langsung terbit di halaman ini.",
            },
          ].map((l) => (
      ...
      {ringkas.length === 0 ? (
        <Kosong
          ...
          keterangan={`Belum ada laporan kas ${rtTerpilih ? rtTerpilih.nama : lingkup.nama} tahun ${tahun} yang selesai melewati persetujuan Ketua RT dan Ketua RW.`}
        />
      ) : (
        <>
              ...
              <BatangGanda
                ...
                keterangan={
                  rtTerpilih
                    ...
                    : lingkup.rw
                      ? `Gabungan seluruh RT di ${lingkup.nama}.`
                      : "Gabungan seluruh RT dari ketiga RW."
                }
              />
            )}
            {!rtDipakai && saldoPerRt.size > 0 && (
              <BatangHorizontal
                ...
                <tbody>
                  {ringkas.map((r) => (
                    <tr key={r.id} className="border-t border-garis hover:bg-kertas/60">
                      ...
                      {!lingkup.rw && (
                        <td className="px-4 py-3 text-slate-600">{r.rw.nama}</td>
                      )}
